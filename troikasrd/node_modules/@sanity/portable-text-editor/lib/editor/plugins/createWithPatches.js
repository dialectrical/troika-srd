...
function createWithPatches(_ref, change$, portableTextFeatures, incomingPatche$) {
...
function adjustSelection(editor, patch, previousChildren) {
  ...
  if (patch.type === 'unset' && patch.path.length === 3) {
    ...
    if (selection.focus.path[0] === _blockIndex) {
  ...
  if (patch.type === 'unset' && patch.path.length === 1) {
    ...
    if (!_block2 || typeof _blockIndex2 === 'undefined') {
      debug('no block found in editor trying to adjust selection for unset block'); // Naively try to adjust as the block above us have been removed.
    ...
    if (_slate.Path.isAfter(selection.anchor.path, [_blockIndex2])) {
      _newSelection2.anchor = _objectSpread({}, selection.anchor);
    ...
    if (_slate.Path.isAfter(selection.focus.path, [_blockIndex2])) {
      _newSelection2.focus = _objectSpread({}, selection.focus);
