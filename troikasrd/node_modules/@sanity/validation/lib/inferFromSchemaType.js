...
function inferFromSchemaType(typeDef, schema) {
  ...
  if (typeDef.options && typeDef.options.list && Array.isArray(typeDef.options.list)) {
    base = base.valid( // eslint-disable-next-line no-warning-comments
    // @ts-expect-error TODO (eventually): fix options list grabbing
...
function inferValidation(field, baseRule) {
  ...
  var validation = typeof field.validation === 'function' ? field.validation(baseRule) : field.validation;
